...
브라우저 안에서 사실 검증을 자동화하는 풀스택 프로젝트입니다. FastAPI 백엔드는 Google Gemini와 자체 모델을 활용해 텍스트·이미지·영상의 진위 여부를 판별하고, **클라이언트는 Chrome 확장 프로그램**을 통해 제공되어 사용자의 탐색 흐름 속에서 곧바로 Fact Check를 실행합니다. React 기반 공유 뷰어는 검증 결과를 외부와 공유할 때 참조 페이지 역할을 합니다.
