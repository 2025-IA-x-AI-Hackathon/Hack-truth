...
const ShareResultPage = () => {
  ...
  useEffect(() => {
    if (location.pathname === '/share') {
      ...
      if (id) {
        sessionStorage.setItem('hacktruth:lastShareUrl', `${location.pathname}${location.search}`);
        return;
      }
    }
    sessionStorage.removeItem('hacktruth:lastShareUrl');
  }, [location]);

  return (
    <div className="share-result-page">
      ...
      {state.status === 'error' && (
        <section className="app-state app-state--error">
          <h2>결과를 가져오지 못했어요</h2>
          <p>{state.error}</p>
          <p className="app-state__hint">링크가 올바른지 또는 잠시 후 다시 시도해주세요.</p>
        </section>
      )}

      {state.status === 'success' && state.data && <ShareResult data={state.data} />}
    </div>
  );
};
